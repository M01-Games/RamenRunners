...
public class Enemy_FPS : MonoBehaviour
{
    ...
    void Update()
    {
        if(Time.time > attackDelay) //This makes the enemy wait after they attack the player
        {
            ...
            {
                //playerObject.GetComponent<PlayerHealth_FPS>().playerHealth -= damageAmount; //This is getting the players health from the other script and taking away the damage
